...
\begin{frame}{From discrete to continuous}
    ...
    \keyeq{\Delta N_x = N_{x, t+1}-N_{x, t} = d[(N_{x-1, t}-N_{x, t}) + N_{x+1, t}-N_{x, t})]}
...
\begin{frame}{Redistribution kernel}
    ...
    \keyeq{k(x,x',\tau,t) = \frac{1}{\rho}(\rho)K_{\tau}(\phi,\hat{\phi})}
    
    where 
    \begin{itemize}
        \item $\rho$ is the distance between locations
        \item $\phi$ is the angle between starting point and finishing point
        \item $\hat{\phi}$ is the direction of the home range center from the current position
    \end{itemize}
...
\begin{frame}{Equation for space use}
    ...
    \keyeq{\frac{dN(x.t)}{dt} = \underbrace{- \triangledown CN(x,t)}{advection} + \underbrace{\triangledown^2 DN(x,t)}{diffusion}}
